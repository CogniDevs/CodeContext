...
class UpdateCheckerThread(QThread):
    ...
    def run(self):
        try:
            ...
            headers = {"User-Agent": "CodeContext-Updater"}
...
def perform_self_update(download_url: str):
    try:
        ...
        if sys.platform == "win32":
            temp_name = "CodeContext_update_temp.exe"
        elif sys.platform.startswith("linux"):
            temp_name = "CodeContext_update_temp.tar.gz"
        elif sys.platform == "darwin":
            temp_name = "CodeContext_update_temp.zip"
        else:
            temp_name = "CodeContext_update_temp"
        ...
        if sys.platform == "win32":
            ...
            new_binary_path = os.path.join(app_dir, "CodeContext_new")
            try:
                with tarfile.open(temp_download_path, "r:gz") as tar:
                    member = tar.getmember("CodeContext")
            ...
            try:
                extract_target = os.path.join(app_dir, "CodeContext_Updated")
                ...
                return True, (
                    ...
                    "Пожалуйста, закройте текущую программу и запустите новую версию CodeContext.app из открывшегося окна Finder."
                ), ""
            except Exception as e:
...
def apply_restart_and_exit(new_file_path: str):
    try:
        current_exe = os.path.abspath(sys.argv[0])
